...
export async function signUpClient(data: ClientSignUpData): Promise<void> {
  ...
  if (userId) {
    const { error: profileError } = await supabase
      ...
      .upsert(
        { id: userId, full_name: trimmedName, email: trimmedEmail, role: "client", account_status: "active", active_role: true },
        { onConflict: "id" }
      );
    if (profileError) {
      console.error("[client-auth] profile upsert:", profileError.message);
...
export async function signInClient(data: ClientSignInData): Promise<void> {
  ...
  if (!profile) {
    await supabase.auth.signOut();
    throw new Error("No client account found. Please sign up or use the Pro login.");
  }
